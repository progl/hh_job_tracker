...
- **Live-прогресс** на `/search`: покрытие индексации обновляется без перезагрузки (SSE `/api/tasks/stream` + поллинг во время backfill/индексации)
...
- **Настраиваемые категории** что слать: новые вакансии с высоким match, приглашения/собесы, ошибки задач, завершение задач, **ежедневный дайджест**
- Завершения задач сливаются в **одно редактируемое сообщение** (анти-спам — правки в Telegram не пингуют)
- **Ежедневный дайджест** в настраиваемый час (на `/profile`): воронка + топ новых вакансий со ссылками
- **Бот-команды**: `/start` (с кнопками), `/status`, `/vacancies`, `/find <запрос>` (семантический поиск), `/run [джоб]`; чувствительные — только из своего chat_id
- Порог match-score и час дайджеста настраиваются в UI; Telegram-токен/chat_id — в `.env` (`TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID`)
