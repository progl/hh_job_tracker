...
- **8 анализаторов** (включаются/отключаются глобально на /profile, можно выбирать на карточке вакансии):
  ...
  - **soft_skills_employer** — тон/культура/WLB работодателя, red/green flags + числовой **soft-score 0–100** (агрегат в колонке `/funnel` и бейджем на вакансии)
  - **cover_letter** — персональное сопроводительное письмо по моему резюме (с кнопкой «скопировать»)
...
- **Cron-джобы**: `llm_parse_requirements` (раз в час) + ручной «Прогнать корпус»; `cover_letter_generate` (раз в 2ч — письма для вакансий «в пайплайне»)
- **CLI** `scripts/llm_parse.py vid [--models a b c]` — сравнение моделей для отладки

### 🧲 RAG — похожие вакансии, семантический поиск, Q&A (опционально)
- Включается отдельным extra: `uv sync --extra rag` (ставит `sqlite-vec`). Без него фичи аккуратно выключены, остальное приложение работает как обычно
- **Эмбеддинги** вакансий через Ollama `nomic-embed-text`, вектора в `sqlite-vec` (vec0, косинус)
- **`/search`** — семантический поиск по корпусу + **Q&A** (вопрос → находим релевантные вакансии → LLM отвечает с ссылками `[#id]`)
- Блок **«🧲 Похожие вакансии»** на странице вакансии
- **Cron-джоб** `embed_vacancies` (раз в 30 мин) + ручная кнопка «индексировать корпус»; покрытие видно на `/search`
- Эмбеддинг строится из **описания** — вакансии из списков поиска без полного текста дотягиваются кнопкой «📥 Дотянуть описания» на `/search` и cron-джобом `backfill_descriptions` (мягко, с остановкой на anti-bot)
- Подробнее — [docs/rag.md](docs/rag.md)

### 🔔 Уведомления
- Каналы: **macOS** (osascript, локально) и **Telegram** (бот). Включаются независимо на `/profile`
- **Настраиваемые категории** что слать: новые вакансии с высоким match, приглашения/собесы, **ошибки фоновых задач**, **завершение задач**
- Порог match-score настраивается в UI; Telegram-токен/chat_id — в `.env` (`TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID`)
...
### Аналитика и логи
- **`/analytics`** — топ-стек, по строгости/категории, тип компании, топ-вопросов и тем к собесу (с сортировкой таблиц)
- **`/jobs`** — журнал прогонов всех фоновых джобов с фильтрами, сортировкой колонок, **кнопкой ⏹ стоп** для running-прогона и авто-обновлением
- **`/searches`** — редактируемая таблица сохранённых поисков: inline-правка имени/текста/региона/remote/глубины/активности + bulk-операции
...
Затем проиндексировать корпус (кнопка на `/search` или cron `embed_vacancies`). Без extra `rag` приложение работает как обычно — RAG-фичи просто выключены. Подробнее — [docs/rag.md](docs/rag.md).
...
| RAG: эмбеддинги, sqlite-vec, поиск и Q&A | [docs/rag.md](docs/rag.md) |
