...
def _assert_not_real_db() -> None:
    ...
    if real == demo:
        raise RuntimeError(
            f"DEMO_DB совпадает с реальной БД ({real}). Прекращаю чтобы не перетереть данные."
        )
    if demo.name in ("hh.db",) or demo.parent.name == "" or demo.parent != Path(__file__).resolve().parent.parent / "data":
...
EMPLOYERS = [
    {"id": 1001, "name": "ООО Ромашка", "is_accredited_it": 1, "all_topic_count": 120, "read_topic_percent": 85, "reply_working_days": 1.5},
    {"id": 1002, "name": "Технополис", "is_accredited_it": 1, "all_topic_count": 450, "read_topic_percent": 92, "reply_working_days": 0.8},
    {"id": 1003, "name": "АйТи Лаб", "is_accredited_it": 0, "all_topic_count": 30, "read_topic_percent": 60, "reply_working_days": 3.0},
    {"id": 1004, "name": "Маркетплейс ШОП", "is_accredited_it": 1, "all_topic_count": 800, "read_topic_percent": 75, "reply_working_days": 2.0},
    {"id": 1005, "name": "Финтех Плюс", "is_accredited_it": 1, "all_topic_count": 200, "read_topic_percent": 50, "reply_working_days": 5.0},
    {"id": 1006, "name": "Облачные Решения", "is_accredited_it": 1, "all_topic_count": 60, "read_topic_percent": 95, "reply_working_days": 0.5},
    {"id": 1007, "name": "Геймдев Студио", "is_accredited_it": 0, "all_topic_count": 15, "read_topic_percent": 40, "reply_working_days": 7.0},
    {"id": 1008, "name": "ЕдТех Платформа", "is_accredited_it": 1, "all_topic_count": 95, "read_topic_percent": 70, "reply_working_days": 2.5},
]

# (id, name, employer_id, area, salary_from, salary_to, is_remote, level, stack, archived, disappeared)
VACANCIES = [
    (90000001, "Senior Python Developer (Django/DRF)", 1001, "Москва", 280000, 350000, 1, "senior", ["python", "django", "drf", "postgresql", "redis"], False, False),
    (90000002, "Python Backend Engineer (FastAPI)", 1002, "Удалённо", 300000, 400000, 1, "senior", ["python", "fastapi", "asyncio", "postgresql", "docker"], False, False),
    (90000003, "Middle Python Developer", 1003, "Санкт-Петербург", 200000, 260000, 0, "middle", ["python", "django", "celery", "rabbitmq"], False, False),
    (90000004, "Lead Python / Tech Lead", 1004, "Москва", 400000, 500000, 1, "lead", ["python", "django", "kubernetes", "microservices"], False, False),
    (90000005, "Senior Python Developer (B2C / Telegram / AI)", 1005, "Удалённо", 300000, 450000, 1, "senior", ["python", "fastapi", "ml"], True, False),
    (90000006, "Backend Developer (Python + ClickHouse)", 1006, "Удалённо", 250000, 330000, 1, "senior", ["python", "clickhouse", "kafka", "docker"], False, False),
    (90000007, "Python Developer (Junior+)", 1007, "Москва", 100000, 150000, 0, "junior", ["python", "flask", "postgresql"], False, False),
    (90000008, "Senior Backend (Python/Go)", 1008, "Удалённо", 320000, 420000, 1, "senior", ["python", "postgresql", "kafka", "microservices"], False, False),
    (90000009, "Python разработчик (highload)", 1002, "Москва", 280000, 380000, 1, "senior", ["python", "asyncio", "redis", "postgresql", "kubernetes"], False, False),
    (90000010, "Middle/Senior Python", 1004, "Удалённо", 220000, 310000, 1, "middle", ["python", "django", "tests"], False, False),
    (90000011, "Python Team Lead", 1001, "Москва", 450000, 550000, 0, "lead", ["python", "django", "postgresql", "aws", "kubernetes"], False, False),
    (90000012, "Senior Python Engineer (ML/Data)", 1003, "Удалённо", 350000, 450000, 1, "senior", ["python", "ml", "postgresql"], False, False),
    (90000013, "Backend Python (RabbitMQ/Celery)", 1005, "Санкт-Петербург", 230000, 290000, 0, "middle", ["python", "django", "celery", "rabbitmq", "redis"], True, False),
    (90000014, "Python Developer (Microservices)", 1006, "Удалённо", 270000, 360000, 1, "senior", ["python", "fastapi", "microservices", "docker", "kubernetes"], False, False),
    (90000015, "Python (FastAPI + Postgres)", 1008, "Удалённо", 240000, 320000, 1, "middle", ["python", "fastapi", "postgresql", "redis"], False, True),
    (90000016, "Senior Python Developer", 1007, "Удалённо", 290000, 380000, 1, "senior", ["python", "django", "postgresql"], False, False),
    (90000017, "Backend Engineer (Python, Linux, Nginx)", 1002, "Москва", 250000, 330000, 0, "middle", ["python", "linux", "nginx", "docker"], False, False),
    (90000018, "Python Senior Developer (search/elasticsearch)", 1004, "Удалённо", 310000, 410000, 1, "senior", ["python", "elasticsearch", "postgresql", "redis"], False, False),
    (90000019, "Python Developer (стажёр)", 1007, "Санкт-Петербург", 60000, 90000, 0, "intern", ["python", "flask"], False, False),
    (90000020, "Lead Backend Python (платформа)", 1001, "Удалённо", 480000, 600000, 1, "lead", ["python", "kubernetes", "aws", "microservices", "kafka"], False, True),
]
...
async def seed(force: bool) -> None:
    ...
    for vid, name, eid, area, sf, st, rem, lvl, stack, archived, disappeared in VACANCIES:
        await conn.execute(
            ...
            (
                vid, name, eid, next(e["name"] for e in EMPLOYERS if e["id"] == eid),
                area,
                sf, st, (sf + st) // 2,
                "remote" if rem else "fullDay",
                rem, rem, lvl,
                json.dumps(stack, ensure_ascii=False),
                10 + vid % 30, 50 + vid % 200, vid % 8,
                ", ".join(stack), f"https://hh.ru/vacancy/{vid}",
    ...
    print(f"  profile: 1, employers: {len(EMPLOYERS)}, vacancies: {len(VACANCIES)}, "
