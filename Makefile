.PHONY: sync run db-init clean demo-seed demo-run demo-clean test coverage

sync:
	uv sync

run:
	uv run uvicorn app.web.app:app --reload --host 127.0.0.1 --port 8000
...
demo-run:
	DB_PATH=data/hh_demo.db uv run uvicorn app.web.app:app --reload --host 127.0.0.1 --port 8000
